...
contract spruikToken is Ownable, StandardToken, Pausable {
 using SafeMath for uint;
    /*
     *  Terminology:
     *  1 token unit = spk
     *  1 token = Spk = Spk * multiplier
     ...
     *  Token metadata
     */
    string constant public name = "Spk Token";
    string constant public symbol = "Spk";
    ...
    */
    ...
    /// @param _preallocated Number of initially provided token units for preallocation (Spk).
...
require(_owner_address != 0x0);
    ...
    /// @dev Allows to destroy token units(Spk).
    /// @param _num Number of token units (Spk) to burn.
